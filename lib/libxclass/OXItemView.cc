...
void OXItemView::DrawDragOutline() {
  ...
  start.y = oxmax(start.y, 0);
  start.x = oxmax(start.x, 0);
  start.y = oxmin(start.y, _canvas->GetHeight());
  start.x = oxmin(start.x, _canvas->GetWidth());

  end.y = oxmax(end.y, 0);
  end.x = oxmax(end.x, 0);
  end.y = oxmin(end.y, _canvas->GetHeight());
  end.x = oxmin(end.x, _canvas->GetWidth());
...
void OXItemView::SelectToPos(OPosition mousePos) {
  ...
  if (_dragSelecting) {
    _selDragStart.x = oxmin(_selAnchor.x, mousePos.x);
    _selDragEnd.x = oxmax(_selAnchor.x, mousePos.x);
    _selDragStart.y = oxmin(_selAnchor.y, mousePos.y);
    _selDragEnd.y = oxmax(_selAnchor.y, mousePos.y);
