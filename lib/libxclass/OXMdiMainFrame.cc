...
ODimension OXMdiContainer::GetDefaultSize() const {
  ...
  return ODimension(oxmax(xpos + _w, rect.right_bottom().x + 1),
                    oxmax(ypos + _h, rect.right_bottom().y + 1));
}
...
int OXMdiContainer::HandleConfigureNotify(XConfigureEvent *event) {
  if (event->window != _id) {
    ...
    int w = oxmax(vw, rect.right_bottom().x + 1);
    int h = oxmax(vh, rect.right_bottom().y + 1);
