...
ODimension OXToolBarButton::GetDefaultSize() const {
  ...
  if (_normalPic) {
    size.w = oxmax(size.w, _normalPic->GetWidth());
    size.h = oxmax(size.h, _normalPic->GetHeight());
  }
  if (_onPic) {
    size.w = oxmax(size.w, _onPic->GetWidth());
    size.h = oxmax(size.h, _onPic->GetHeight());
  }
  if (_disabledPic) {
    size.w = oxmax(size.w, _disabledPic->GetWidth());
    size.h = oxmax(size.h, _disabledPic->GetHeight());
  }
  size.w += 6;
  size.h += _th + 6;
