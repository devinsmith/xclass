...
void O2ColumnsLayout::Layout() {
  ...
  for (ptr = *_list; ptr != NULL; ptr = ptr->next) {
    ...
    c1size.w = oxmax(c1size.w, csize.w);
    c1size.h = oxmax(c1size.h, csize.h);
    ptr = ptr->next;
    if (ptr == NULL) break;
    csize = ptr->frame->GetDefaultSize();
    c2size.w = oxmax(c2size.w, csize.w);
    c2size.h = oxmax(c2size.h, csize.h);
  }

  h = oxmax(c1size.h, c2size.h);
...
ODimension O2ColumnsLayout::GetDefaultSize() const {
  ...
  size.h = (oxmax(c1size.h, c2size.h) + _vsep) * count + ins.t + ins.b;
