...
OXHorizontalResizer::OXHorizontalResizer(const OXWindow *p, int wid,
  ...
  OXFrame(p, 4, 2, FIXED_HEIGHT) {
    ...
    curPix  = XCreateBitmapFromData(GetDisplay(), GetId(),
				reinterpret_cast<const char *>(HResizer_bits),
                                    HResizer_width, HResizer_height);
    maskPix = XCreateBitmapFromData(GetDisplay(), GetId(), reinterpret_cast<const char *>(HmaskResizer_bits),
...
OXVerticalResizer::OXVerticalResizer(const OXWindow *p, int wid,
  ...
  OXFrame(p, 2, 4, FIXED_WIDTH) {
    ...
    curPix = XCreateBitmapFromData(GetDisplay(), GetId(),
        reinterpret_cast<const char *>(VResizer_bits),
                                   VResizer_width, VResizer_height);
    maskPix = XCreateBitmapFromData(GetDisplay(), GetId(), reinterpret_cast<const char *>(VmaskResizer_bits),
