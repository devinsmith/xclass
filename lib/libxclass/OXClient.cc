...
#if 1
  color.red   = oxmax((white_p.red/5),   color.red);
  color.green = oxmax((white_p.green/5), color.green);
  color.blue  = oxmax((white_p.blue/5),  color.blue);
  
  color.red   = oxmin(white_p.red,   (color.red*140)/100);
  color.green = oxmin(white_p.green, (color.green*140)/100);
  color.blue  = oxmin(white_p.blue,  (color.blue*140)/100);
#else
...
const OPicture *OXClient::GetPicture(const char *name, const char **data) {
