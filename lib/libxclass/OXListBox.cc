...
void OXListBox::AddEntry(OString *s, int ID) {
  ...
  _itemVsize = oxmax(_itemVsize, lbe->GetDefaultHeight());
...
void OXListBox::Resize(int w, int h) {
  if (_integralHeight) {
    ...
    h = oxmax(_itemVsize, ((h - ih) / _itemVsize) * _itemVsize) + ih;
  }
...
ODimension OXListBox::GetDefaultSize() const {
  ...
  if (_integralHeight) {
    int ih = _insets.t + _insets.b;
    h = oxmax(_itemVsize, ((_h - ih) / _itemVsize) * _itemVsize) + ih;
  } else {
    h = _h;
  }
...
void OXListBox::Layout() {
  ...
  _container->Layout();
  tch = oxmax(_container->GetDefaultHeight(), ch);
