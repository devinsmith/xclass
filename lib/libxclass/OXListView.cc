  ...
  if ((_viewMode == LV_DETAILS) && (columnData.size() > 0)) {
    ...
    if (_fullRowSelect) {
      ...
      w->FillRectangle(rectGC, pos.x + _textPos.x, pos.y + _textPos.y,
                       oxmin(_tw, columnData[0]->width - _textPos.x - 2), _th + 1);
    }
    ...
    if (_focused && (_tw > 0)) {
      if (_fullRowSelect) {
        ...
        w->DrawRectangle(_client->GetResourcePool()->GetDocumentBckgndGC()->GetGC(),
                         ...
                         oxmin(_tw, columnData[0]->width - _textPos.x - 2)-1, _th);
        w->DrawRectangle(_client->GetResourcePool()->GetFocusHiliteGC()->GetGC(),
                         pos.x + _textPos.x, pos.y + _textPos.y,
                         oxmin(_tw, columnData[0]->width - _textPos.x - 2)-1, _th);
      }
...
int OXListView::HandleKey(XKeyEvent *event) {
  ...
  if (event->type == KeyPress) {
    ...
    KeySym keysym = XkbKeycodeToKeysym(GetDisplay(), event->keycode, 0, 0);
    switch (keysym) {
      case XK_Left:
        _KeyLeft();
        break;
...
bool OXListView::ItemLayout() {
  ...
  if (_items.size() > 0) {
    ...
    case LV_LARGE_ICONS:
      ...
      _virtualSize.w = oxmax(canvasSize.w, maxItemSize.w + _itemSep.w * 2);
      _rows = 0;
      for (i = 0; i < _items.size(); i++) {
    ...
    case LV_LIST:
      currentPos.x = _itemSep.w;
      currentPos.y = _itemSep.h;
      _virtualSize.h = oxmax(canvasSize.h, maxItemSize.h + _itemSep.h * 2);
      _cols = 0;
      for (i = 0; i < _items.size(); i++) {
