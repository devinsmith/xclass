...
void OTileLayout::Layout() {
  ...
  for (ptr=*_list; ptr != NULL; ptr=ptr->next) {
    ...
    max_osize.w = oxmax(max_osize.w, csize.w);
    max_osize.h = oxmax(max_osize.h, csize.h);
  }

  max_width = oxmax(msize.w, max_osize.w + (_sep << 1));
  x = _sep; y = _sep << 1;
  for (ptr=*_list; ptr != NULL; ptr=ptr->next) {
...
ODimension OTileLayout::GetDefaultSize() const {
  ...
  for (ptr=*_list; ptr != NULL; ptr=ptr->next) {
    ...
    max_osize.w = oxmax(max_osize.w, max_size.w);
    max_osize.h = oxmax(max_osize.h, max_size.h);
  }

  max_size.w = oxmax(msize.w, max_osize.w + (_sep << 1));
...
void OListLayout::Layout() {
  ...
  max_height = oxmax(msize.h, max_osize.h + (_sep << 1));
...
ODimension OListLayout::GetDefaultSize() const {
  ...
  max_size.h = oxmax(msize.h, max_osize.h + (_sep << 1));
