...
static const char *defaultFiletypes[] = { "All files",      "*",
                                    "Document files", "*.doc",
                                    "Text files",     "*.txt",
                                    NULL,             NULL };
...
void OXFileDialog::_FileDialog(int dlg_type, OFileInfo *file_info) {
  ...
  const char *lookinString;
  const char *okButtonString;
  ...
  int width = oxmax(_ok->GetDefaultWidth(), _cancel->GetDefaultWidth()) + 20;
...
int OXFileDialog::HandleKey(XKeyEvent *event) {
  if ((event->type == KeyPress) && (event->state & Mod1Mask)) {
    int keysym = XkbKeycodeToKeysym(GetDisplay(), event->keycode, 0, 0);
    switch (keysym) {
      ...
      case IDF_ADDFAVOR:
        if (_dlg_type & FDLG_FAVOURITES) {  // paranoia check
          if (_fv->NumSelected() == 1) { // otherwise use current dir?
            char path[PATH_MAX], oldpath[PATH_MAX], tmp[8192];
