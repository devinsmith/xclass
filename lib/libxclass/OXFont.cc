...
struct StateMap { int numKey; const char *strKey; };
...
const char *OFontPool::FindStateString(const StateMap *map, int numKey) {
...
int OXFont::PostscriptFontName(OString *dst) const {
  const char *family;
  const char *weightString, *slantString;
  char *src, *dest;
  int upper, len;
